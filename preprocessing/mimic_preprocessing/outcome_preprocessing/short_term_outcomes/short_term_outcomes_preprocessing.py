from preprocessing.mimic_preprocessing.outcome_preprocessing.short_term_outcomes.early_neurological_deterioration import \
...
def preprocess_short_term_outcomes(df, end_require_min_repeats=False, end_min_delta=4, end_keep_multiple_events=True):
    ...
    end_df = early_neurological_deterioration(
        df,
        require_min_repeats=end_require_min_repeats,
        min_delta=end_min_delta,
        keep_multiple_events=end_keep_multiple_events,
    )
    end_df['outcome_label'] = 'early_neurological_deterioration'
    # store arguments for each outcome
    end_df['outcome_args'] = (
        f'require_min_repeats={end_require_min_repeats}, '
        f'min_delta={end_min_delta}, '
        f'keep_multiple_events={end_keep_multiple_events}'
    )
