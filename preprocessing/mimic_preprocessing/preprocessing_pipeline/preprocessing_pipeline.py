...
from preprocessing.mimic_preprocessing.outcome_preprocessing.long_term_outcomes.outcome_preprocessing import preprocess_outcomes
from preprocessing.mimic_preprocessing.outcome_preprocessing.short_term_outcomes.short_term_outcomes_preprocessing import \
...
def preprocess(
    ...
    log_dir: str = '',
    include_short_term_outcomes: bool = True,
    short_term_outcomes_config: dict = {},
    verbose: bool = True,
    desired_time_range: int = 72,
) -> pd.DataFrame:
    """
    ...
    preprocessed_long_term_outcomes_df = preprocess_outcomes(outcome_table_path, verbose=verbose)
    if include_short_term_outcomes:
        preprocessed_short_term_outcomes_df = preprocess_short_term_outcomes(
            restricted_feature_df,
...
def preprocess_and_save(
    ...
    outcome_file_prefix: str = 'preprocessed_outcomes',
    verbose: bool = True,
):
    ...
    preprocessed_feature_df, preprocessed_outcomes = preprocess(
        extracted_tables_path,
        admission_notes_data_path,
        reference_population_imputation_path,
        reference_population_normalisation_parameters_path,
        reference_categorical_encoding_path,
        preproccessed_monitoring_data_path,
        mimic_admission_nihss_db_path,
        log_dir=log_dir,
    ...
    if include_short_term_outcomes:
        ...
        short_term_outcomes_path = os.path.join(
            output_dir,
            f'{outcome_file_prefix}_short_term_{timestamp}.csv',
        )
        preprocessed_short_term_outcome_df.to_csv(short_term_outcomes_path, index=False)
    else:
        preprocessed_long_term_outcome_df = preprocessed_outcomes[0]

    preprocessed_long_term_outcome_df.to_csv(outcomes_save_path, index=False)
    ...
    outcome_presence_verification(
        preprocessed_long_term_outcome_df,
        preprocessed_feature_df,
        log_dir=log_dir,
        outcomes=['Death in hospital', '3M Death'],
    )
...
if __name__ == '__main__':
    ...
    preprocess_and_save(args.ehr_tables_path, args.admission_notes_path,
                        ...
                        args.output_dir,
                        args.preprocessed_monitoring_path, args.mimic_admission_nihss_db_path,
