...
def normalise_data(df: pd.DataFrame, reference_population_normalisation_parameters_path:str = '',
                   winsorize:bool = True, verbose:bool = True, log_dir: str = '') -> pd.DataFrame:
    """
    ...
    if winsorize:
        ...
        normalized_feature_df = winsorized_restricted_feature_df.copy()
    else:
        normalized_feature_df = df.copy()
    ...
    if verbose:
        print('Normalising...')
    for variable in tqdm(variables_to_normalize):
        temp = normalized_feature_df[
            normalized_feature_df.sample_label == variable].value.copy()
        ...
        normalized_feature_df.loc[
            normalized_feature_df.sample_label == variable, 'value'] = temp
