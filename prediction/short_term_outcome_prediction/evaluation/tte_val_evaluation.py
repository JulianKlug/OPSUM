...
from prediction.outcome_prediction.Transformer.lightning_wrapper import LitModel, LitEncoderRegressionModel
...
def validation_evaluation(data_path:str, model_config_path:str, model_path:str=None,
    ...
    if output_path is None:
        output_path = os.path.join(os.path.dirname(model_config_path),
                                   f'tte_validation_evaluation_results_{eval_n_time_steps_before_event}h')
    ...
    # else load model and compute predictions
        ...
        trained_model = LitEncoderRegressionModel.load_from_checkpoint(checkpoint_path=model_path,
        ...
        # save predictions
        ch.save(pred_over_ts, os.path.join(output_path, 'predictions.pt'))
    ...
    # compute roc / auprc / accuracy / mcc scores for each time step as well as their binary versions
    roc_scores = []
    auprc_scores = []
    mcc_scores = []
    ...
    all_balanced_scores = [roc_scores, auprc_scores, mcc_scores, binary_roc_scores, binary_auprc_scores]
    n_pos_samples = []
    ...
    for ts in range(n_time_steps):
        ...
        y_pred_binary = y_pred < eval_n_time_steps_before_event
        # because time to event is predicted, we need to reverse the non binary prediction
        inv_y_pred = -1 * y_pred
        ...
        if len(np.unique(y_true)) == 1:
            for score in all_balanced_scores:
                score.append(np.nan)
        else:
            roc_scores.append(roc_auc_score(y_true, inv_y_pred))
            auprc_scores.append(average_precision_score(y_true, inv_y_pred))
            mcc_scores.append(matthews_corrcoef(y_true, y_pred_binary))
            binary_roc_scores.append(roc_auc_score(y_true, y_pred_binary))
            binary_auprc_scores.append(average_precision_score(y_true, y_pred_binary))
    
    # median over time for each metric
    results_df = pd.DataFrame({'roc': np.nanmedian(roc_scores),
                                    'auprc': np.nanmedian(auprc_scores),
                                    'mcc': np.nanmedian(mcc_scores),
                                    'accuracy': np.nanmedian(accuracy_scores),
                                    'binary_roc': np.nanmedian(binary_roc_scores),
                                    'binary_auprc': np.nanmedian(binary_auprc_scores),
    ...
    pd.DataFrame({'roc': roc_scores, 'auprc': auprc_scores, 'mcc': mcc_scores,
                    'accuracy': accuracy_scores, 'binary_roc': binary_roc_scores,
                    'binary_auprc': binary_auprc_scores, 'n_pos_samples': n_pos_samples
                  }).to_csv(
    ...
    sns.scatterplot(x=range(1, n_time_steps + 1), y=accuracy_scores, label='Accuracy', ax=ax)
