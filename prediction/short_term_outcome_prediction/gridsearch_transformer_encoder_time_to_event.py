...
def launch_gridsearch_encoder_time_to_event(data_splits_path:str, output_folder:str, gridsearch_config:dict=DEFAULT_GRIDEARCH_CONFIG, use_gpu:bool=True,
    ...
    output_folder = path.join(output_folder, f'transformer_tte_gs_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ensure_dir(output_folder)
...
def get_score_encoder_tte(trial, ds, data_splits_path, output_folder, gridsearch_config:dict=DEFAULT_GRIDEARCH_CONFIG, use_gpu=True):
    ...
    for i, (train_dataset, val_dataset) in enumerate(ds):
        ...
        trainer = pl.Trainer(accelerator=accelerator, devices=1, max_epochs=gridsearch_config['max_epochs'],
                             ...
                             callbacks=[MyEarlyStopping(step_limit=early_stopping_step_limit, metric='val_mae'), checkpoint_callback],
                             gradient_clip_val=grad_clip)
        trainer.fit(model=module, train_dataloaders=train_loader, val_dataloaders=val_loader)

        val_maes = np.array([x['val_mae'] for x in logger.metrics if 'val_mae' in x])
        val_mapes = np.array([x['val_mape'] for x in logger.metrics if 'val_mape' in x])
        best_idx = np.argmin(val_maes)
        rolling_val_mae = np.median(val_maes[max(0, best_idx - 1): best_idx + 2])
        rolling_val_mape = np.median(val_mapes[max(0, best_idx - 1): best_idx + 2])

        best_val_mae = np.min([x['val_mae'] for x in logger.metrics if 'val_mae' in x])
        best_epoch = np.argmin([x['val_mae'] for x in logger.metrics if 'val_mae' in x])
        # mape at best epoch
        best_val_mape = [x['val_mape'] for x in logger.metrics if 'val_mape' in x][best_epoch]

        val_mae_scores.append(best_val_mae)
        val_mape_scores.append(best_val_mape)
    ...
    return np.median(rolling_val_maes)
...
if __name__ == '__main__':
    ...
    launch_gridsearch_encoder_time_to_event(data_splits_path=args.data_splits_path, output_folder=args.output_folder, gridsearch_config=gridsearch_config,
