...
class ModelWithTemperature(nn.Module):
    ...
    """
    def __init__(self, model, use_gpu=True, verbose=False, target_class=-1):
        super(ModelWithTemperature, self).__init__()
        self.model = model
        self.temperature = nn.Parameter(ch.ones(1) * 1.5)
    ...
    def forward(self, input):
        logits = self.model(input)[:,:self.target_class]
    ...
    def set_temperature(self, valid_loader):
        ...
        with ch.no_grad():
            ...
            if self.use_gpu:
                logits = ch.cat(logits_list).cuda()
                labels = ch.cat(labels_list).cuda()
            else:
                logits = ch.cat(logits_list)
                labels = ch.cat(labels_list)

        # add axis
        logits = logits[:, self.target_class].squeeze()
        # cross entropy loss expects shape N, C (num_examples, num_classes)
        logits = ch.stack([1 - logits, logits], -1)
        labels = labels.long()
...
class _ECELoss(nn.Module):
    ...
    def __init__(self, n_bins=15):
        ...
        bin_boundaries = ch.linspace(0, 1, n_bins + 1)
        self.bin_lowers = bin_boundaries[:-1]
        self.bin_uppers = bin_boundaries[1:]

    def forward(self, logits, labels):
        softmaxes = F.softmax(logits, dim=1)
        confidences, predictions = ch.max(softmaxes, 1)
        accuracies = predictions.eq(labels)

        ece = ch.zeros(1, device=logits.device)
        for bin_lower, bin_upper in zip(self.bin_lowers, self.bin_uppers):
            ...
            if prop_in_bin.item() > 0:
                ...
                ece += ch.abs(avg_confidence_in_bin - accuracy_in_bin) * prop_in_bin
